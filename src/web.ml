...
type http_response =
  {status_code: Status.t; body: string; headers: (string * string) list}
...
type 'a http_context =
  {request: http_request; response: http_response; state: 'a; continue: bool}
...
let middleware_combine (first_filter: 'a web_server ) (second_filter: 'a web_server ) (ctx: 'a http_context) =
  ...
  let res = match result1 with
    ...
    | Some first_result_ctx -> 
      if first_result_ctx.continue then
        second_filter first_result_ctx 
      else 
        Some first_result_ctx |> Lwt.return 
  in
  res
...
let bad_request body (ctx : 'a http_context) =
  let modified_response =
    {ctx.response with body; status_code= `Bad_request}
  in
...
module Helpers = struct
  let as_json ctx =
    ...
    in
    let new_ctx =
      {ctx with response= {ctx.response with headers= updated_headers}}
    in
    new_ctx |> Lwt.return
end
...
module Router = struct
  ...
  let pull_path_param str =
    if String.length str > 1 then Caml.String.sub str 1 (String.length str - 1)
    else str
  ...
  let rec parse_whiles_going_right structured_list path_segment_list
      extracted_path_params_list =
    match (List.hd structured_list, List.hd path_segment_list) with
    | Some _, None -> (false, [])
    | None, Some _ -> (false, [])
    | None, None -> (true, extracted_path_params_list)
    | Some a, Some b ->
        if a = b && List.length structured_list = 1 then
          (true, extracted_path_params_list)
        else if a = b then
  ...
  let parse_route (route_format : string) (in_url : string)
      (ctx : 'a http_context) =
    ...
    let url, _query_params =
      ...
      | Some pos ->
          ...
          let query_params_str =
            Caml.String.sub in_url pos (String.length in_url)
          in
    ...
    let broken_structure =
      String.split ~on:'/' route_format |> remove_empty_strings
    in
    ...
    | true ->
        let is_match, path_params =
          parse_whiles_going_right broken_structure broken_url []
        in
        if is_match = true then
          let updated_context =
            {ctx with request= {ctx.request with path_params}}
          in
...
let respond_with_text reqd status result =
  let headers =
    Headers.of_list
      ( [ ("content-length", string_of_int (get_content_length result.body))
        ]
      @ result.headers )
  in
...
let make_cust_router (routes : (Httpaf.Method.t * string * 'a web_server) list)
    app_state =
  let request_handler _ reqd =
    ...
    >|= (fun body ->
          ...
          let initial_ctx =
            make_ctx app_state req.Request.meth req.target (Some body)
          in
          let found_handler =
          ...
          | Some (_, structure, handler) -> (
              ...
              let _, updated_context =
                Router.parse_route structure req.target initial_ctx
              in
              let result' = updated_context |> handler in
              let out = result' >|= (fun result ->
                match result with
                | None -> respond_with_text reqd `Not_found initial_ctx.response
                | Some r ->
                    respond_with_text reqd r.response.status_code r.response 
                )
                in out |> ignore
              ))
    |> ignore
